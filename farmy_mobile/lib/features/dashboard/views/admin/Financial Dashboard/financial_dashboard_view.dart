...
class _FinancialDashboardViewState extends State<FinancialDashboardView>
  ...
  double _adminSubtractKg = 0.0; // field entered by admin to subtract
  ...
  // Inventory tab: load both distribution and loading net weights for a date
  Future<void> _loadInventoryForDate(DateTime date) async {
    setState(() {
      _inventoryLoading = true;
    });
    try {
      // Distribution daily net
      ...
      // Loading daily net from loadings stats using same-day range
            ...
            if (_inventoryLoading)
              ...
              Column(
                children: [
                  ...
                  const SizedBox(height: 16),
                  // Admin adjustable field (to be subtracted)
                  Card(
                    child: Padding(
                      padding: const EdgeInsets.all(16),
                      child: Column(
  ...
  Widget _inventorySummaryCard() {
    final double difference = _dailyLoadingNet - _dailyDistributionNet; // الفرق
    final double stock = difference - _adminSubtractKg; // المخزون اليومي
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
